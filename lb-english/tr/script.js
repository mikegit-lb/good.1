...
window.openLead = (source)=>{
  ...
  leadKicker.textContent = 'Ücretsiz Kaynak';
  leadTitle.textContent = source;
  leadDesc.textContent = 'E-postanı gir — anında indirme, bir faydalı takip. Spam yok. KVKK uyumlu.';
  leadModal.showModal();
  setTimeout(()=> document.getElementById('leadEmail')?.focus(), 80);
}
window.openCheckout = (plan)=>{
  const p = yearly ? 'Yearly' : 'Monthly';
  showToast(`Harika seçim — ${plan} (${p}) — ödeme açılıyor (demo). Yerin 15 dk tutulacak.`);
...
window.handleLead = (e)=>{
  ...
  leadModal.close();
  showToast(`Gönderildi! Kontrol et ${email} — "${source}" için — gelen kutusunda yoksa spam’e bak. L.B. English Co.’ya hoş geldin.`);
  e.target.reset();
}
window.handleBooking = (e)=>{
  ...
  e.target.reset();
  showToast(`Teşekkürler, ${name}! Your free call request — "${goal}" alındı. Slotunu onaylamak için 6 saat içinde e-posta göndereceğiz.`);
  toggleMenu(false);
}
...
const commands = [
  ...
  {id:'resources', label:'Ücretsiz Kaynaks', desc:'IELTS Band 9 PDF • TOEFL audio • Phrase bank', icon:'◈', href:'#resources'},
