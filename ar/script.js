...
window.openLead = (source)=>{
  ...
  leadKicker.textContent = 'مورد مجاني';
  leadTitle.textContent = source;
  leadDesc.textContent = 'أدخل بريدك — تحميل فوري ومتابعة مفيدة واحدة. بلا إزعاج. متوافق مع GDPR.';
  leadModal.showModal();
  setTimeout(()=> document.getElementById('leadEmail')?.focus(), 80);
}
window.openCheckout = (plan)=>{
  const p = yearly ? 'Yearly' : 'Monthly';
  showToast(`اختيار رائع — ${plan} (${p}) — فتح الدفع (تجريبي). سيُحجز مكانك 15 دقيقة.`);
...
window.handleLead = (e)=>{
  ...
  leadModal.close();
  showToast(`تم الإرسال! تحقق من ${email} لـ "${source}" — إذا لم يكن في الوارد، تحقق من الرسائل المزعجة. مرحباً بك في L.B. English Co.`);
  e.target.reset();
}
window.handleBooking = (e)=>{
  ...
  e.target.reset();
  showToast(`شكراً، ${name}! Your free call request لـ "${goal}" . سنرسل بريداً خلال 6 ساعات لتأكيد موعدك.`);
  toggleMenu(false);
}
...
const commands = [
  ...
  {id:'resources', label:'مورد مجانيs', desc:'IELTS Band 9 PDF • TOEFL audio • Phrase bank', icon:'◈', href:'#resources'},
